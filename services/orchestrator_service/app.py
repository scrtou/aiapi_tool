...
from services.orchestrator_service.routes.workflows import router as workflows_router
from services.orchestrator_service.routes.artifacts import router as artifacts_router
from services.orchestrator_service.routes.events import router as events_router
from services.orchestrator_service.service import OrchestratorService


app = FastAPI(title="orchestrator-service")
app.state.service_name = "orchestrator-service"
configure_cors(app)
...
async def attach_trace_id(request: Request, call_next):
    trace_id = request.headers.get("X-Trace-Id") or generate_trace_id("trc_flow")
    try:
...
def health(request: Request):
    return success_response(request.state.trace_id, HealthData(service="orchestrator-service").model_dump(mode="json"))


@app.get("/api/v1/health/details", dependencies=[Depends(require_internal_or_admin())])
def health_details(request: Request):
    data = request.app.state.orchestrator_service.metrics_snapshot()
...
def recover_incomplete_tasks():
    if env_bool("WORKFLOW_ENABLE_STARTUP_RECOVERY", True):
        app.state.orchestrator_service.recover_incomplete_tasks()
        app.state.orchestrator_service.recover_pending_callbacks()
    if env_bool("WORKFLOW_ENABLE_EMBEDDED_WORKER", True):
        app.state.orchestrator_service.start_worker()


@app.on_event("shutdown")
def stop_worker():
    if env_bool("WORKFLOW_ENABLE_EMBEDDED_WORKER", True):
        app.state.orchestrator_service.stop_worker()
